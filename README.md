...
최근 **비동기 이벤트 소싱(Event Sourcing) 기반 PostgreSQL 실시간 자산 정산(Settle) 파이프라인**, **고대비 다크 테마 터미널(JavaF)**, **ADA-KRW 멀티 심볼 병렬 확장**, **진짜 RTT 네트워크 지연 실측**, **rAF 기반 호가창 렌더링 스로틀링**, **Prometheus & Grafana 기반 0-의존성 초경량 성능 계측 체계**, **Spring Boot 기반 통합 어드민 API & 어드민 대시보드 콘솔**, 그리고 **초현대식 React 18 + TypeScript + Vite 기반의 고성능 실시간 관제 어드민 터미널**이 추가 완비됨.
...
*   **완벽한 UI와 로직의 분리 (ES6 모듈화):** 기존 2,200라인에 육박하던 비대한 단일 HTML 파일에서 인라인 자바스크립트 전체를 분리 추출하여 **의존성 제로의 독립적인 ES6 모듈 파일인 [admin.js](file:///home/administrator/exchange_be/frontend/js/admin.js)로 리팩토링**을 완료했습니다. 이를 통해 UI/Style 레이어와 비즈니스 로직 레이어를 완벽히 차단 격리하여 고성능 유지보수 구조를 100% 확보했습니다.
...
### 3. 초현대식 React + TypeScript 고성능 실시간 관제 터미널 ([frontend-react](file:///home/administrator/exchange_be/frontend-react)) (🌟 신규)
...
*   **조치 사항:**
    ...
    *   **다중 시간 해상도 지원 (1M, 5M, 15M, 1H):** 백엔드(`admin-api`) 및 프론트엔드 연동을 확장하여 1분 봉 외에도 5분 봉, 15분 봉, 1시간 봉 단위 해상도를 동적으로 제공합니다. 백엔드의 Java 서비스 계층 메모리 상에서 실시간 시간 분할 내림 연산 그룹핑을 고속 수행하여 dynamic fetch해 줍니다.
...
### 11. 🌌 역사적 체결 10만 건 DB 시딩 및 하이브리드 차트 캔들 보정 패딩 엔진 (이중 보장 아키텍처) (🌟 신규)
...
### 🌌 7. 현대식 React + TypeScript 고성능 실시간 어드민 개발 및 구동 환경 (🌟 신규)
기존 바닐라 프런트엔드(`frontend` 폴더)를 원본 보존한 채, 최신 **React 18 + TS + Vite** 뼈대로 설계된 고성능 독립형 실시간 관제 및 어드민 터미널(`frontend-react`)을 추가 구축하였습니다.

1. **폴더 및 도구 정보:**
   * **프로젝트 폴더:** [frontend-react](file:///home/administrator/exchange_be/frontend-react)
   * **환경:** Node.js v22.12.0 이상 (로컬 샌드박스 `./node-env`에 완전히 포함 배포)
   * **기술 스택:** React 18, Vite 8, TypeScript 5, Zustand (중앙 상태 관리), Lightweight-Charts, Lucide React (네온 아이콘)
...
4. **로컬 개발 서버 구동 방법 (WSL 샌드박스 실행 기준):**
   ```bash
   # 로컬 Node-env PATH를 포함하여 개발 서버 가동
   export PATH=/home/administrator/exchange_be/node-env/bin:$PATH
   cd frontend-react
   npm run dev
   ```
   가동 완료 후 브라우저에서 제공되는 로컬 개발 서버 URL([http://localhost:5173](http://localhost:5173))로 즉각 접속하여 고성능 모니터 실황을 관제하십시오.
...
5. **프로덕션 빌드 번들링 생성:**
   ...
   npm run build
   ```
   컴파일 에러가 0인 컴파일 정합성을 검증하였으며, Nginx 정적 서빙 경로에 올릴 최적의 `dist` 번들 파일이 생성됩니다.
