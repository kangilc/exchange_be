...
*   **조치 사항:**
    ...
    *   cAdvisor 내부 포트는 8080으로 고정이지만, 실시간 게이트웨이(`ws-gateway`)가 이미 호스트의 **8080**을 선점하여 사용하고 있으므로, 호스트 외부 바인딩 포트를 **`8082`**로 우회 매핑하여 포트 충돌을 완벽 방지했습니다.
...
*   **효과:** 마켓이 처음 시작하자마자 매도 25개, 매수 25개의 촘촘하고 넓은 호가 장부가 매치 코어에 적재되어, UI 호가창 20칸 전체가 빈칸(`--`) 없이 실시간 지표들로 꽉 차서 노출되는 극상의 시각적 거래 환경을 완성했습니다.
