...
1. **폴더 및 도구 정보:**
   * **프로젝트 폴더:** [frontend-react](./frontend-react)
   * **환경:** Node.js v22.12.0 이상 (로컬 샌드박스 `./node-env`에 완전히 포함 배포)
   * **기술 스택:** React 19, Vite 8, TypeScript 6, Zustand (중앙 상태 관리), Lightweight-Charts, Lucide React (네온 아이콘)
...
4. **로컬 개발 서버 구동 방법 (WSL 및 Windows OS 공통):**
   * **WSL / Linux (샌드박스 실행):**
     ```bash
     export PATH=./node-env/bin:$PATH
     cd frontend-react
     npm run dev
     ```
   * **Windows (PowerShell):**
