...
- **동적 자산 연동 (JWT 기반 me 조회)**: Access Token 내부에 주입된 `userId` 클레임을 로컬에서 직접 디코딩하여 식별하므로, 기존 403 Forbidden 오류를 유발하던 외부 `/admin/users` 조회 호출이 불필요해졌습니다. 또한 일반 사용자의 지갑 잔고 및 체결/원장 내역은 권한 상승(IDOR) 취약점 차단을 위해 본인 전용 무상태 API인 `/admin/wallets/me`를 호출하도록 고도화하였습니다.
