...
### 4. 실시간 모의 주문 및 자산 연동
- **주문 콘솔**: 지정가(LIMIT) 및 시장가(MARKET) 주문 입력을 지원하며 매수/매도 탭을 제공합니다.
- **호가 연계**: 호가창의 특정 행을 클릭하면 해당 가격이 주문 콘솔의 가격 입력창에 자동으로 채워집니다.
- **지갑 잔고 동기화**: 서버 API `/admin/wallets/user/1`를 통해 자산 잔고를 실시간 연동하며, 주문 제출 시 `/admin/users/1/assets/adjust`를 통해 자산을 가감합니다.
