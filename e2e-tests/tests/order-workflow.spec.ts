...
test('신규 유저 가입 후 주문 체결 및 어드민 수수료 정산 확인', async ({ page }) => {
  ...
  // 2. 공개 회원가입 API를 호출하여 PENDING 상태의 신규 사용자 가입 신청
  ...
  // 3. 승인 전 로그인 시도하여 차단 여부 검증 (PENDING 가드 작동 확인)
  ...
  // 4. 어드민 토큰으로 PENDING 회원을 ACTIVE로 가입 승인 처리 (PUT /admin/users/{id})
  ...
  // 7. ADA-KRW 마켓으로 전환 (보유 자산이 KRW이므로)
  ...
  // 8. 실시간 매칭 로그 콘솔에 주문 전송 패킷 로그가 출력되었는지 확인하여 웹소켓 전송 성공 검증
