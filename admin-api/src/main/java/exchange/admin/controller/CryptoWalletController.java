...
public class CryptoWalletController {
     ...
     *   <li>가용 잔고(balance)에서 출금액을 차감하고, 출금 심사 중 자산 유실을 방지하기 위해 잠금 잔고(lockedBalance)에 임시로 가산합니다.</li>
     *   <li>초기 상태가 {@code PENDING}인 신규 출금 데이터(CryptoWithdrawal)를 생성 및 저장합니다.</li>
     * </ol>
     *
     * @param payload 사용자 ID(userId), 통화(currency), 수량(amount), 대상 주소(toAddress)를 포함하는 맵
     ...
     *   <li>유저의 가상 지갑(Wallet)을 조회하여, 묶여 있던 잠금 잔고(lockedBalance)에서 출금액을 차감하고, 이를 다시 사용 가능한 가용 잔고(balance)로 복원합니다.</li>
     * </ol>
     *
    ...
    public ResponseEntity<?> rejectWithdrawal(@PathVariable Long id) {
        ...
        if (wallet != null) {
            ...
            if (nextLocked.compareTo(BigDecimal.ZERO) < 0) nextLocked = BigDecimal.ZERO; // 음수 방지 예외 처리
