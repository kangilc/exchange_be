...
/**
 * ADA 네트워크 입출금 및 온체인 연동을 담당하는 서비스 구현체.
 * Ganache 로컬 EVM 노드 상에 ADA ERC-20 스마트 계약을 배포하여 온체인으로 운영한다.
 */
@Slf4j
@Service
public class AdaCoinService implements CoinNetworkService {
    ...
    // ADAToken Solidity compiled bytecode (ERC-20 기반 대체 스마트 계약)
    ...
    @PostConstruct
    public void init() {
        // ADA용 독자적 계약 배포를 위해 백그라운드 스레드로 기동한다.
        new Thread(() -> {
            int retries = 10;
            while (retries > 0) {
                try {
                    log.info("[AdaCoinService] 로컬 EVM 노드 연결을 시도합니다... (남은 횟수: {})", retries);
                    String rpcUrl = dockerRpcUrl;
                    try {
                        web3j = Web3j.build(new HttpService(rpcUrl));
                        web3j.web3ClientVersion().send();
                        log.info("[AdaCoinService] Ganache 컨테이너 연결 성공! ({})", rpcUrl);
                    } catch (Exception e) {
                        rpcUrl = defaultRpcUrl;
                        web3j = Web3j.build(new HttpService(rpcUrl));
                        web3j.web3ClientVersion().send();
                        log.info("[AdaCoinService] Ganache 로컬 호스트 연결 성공! ({})", rpcUrl);
                    }
                    ...
                    log.error("[AdaCoinService] 연결 또는 배포 실패: {}. 3초 후 재시도...", e.getMessage());
                    retries--;
                    try {
                        Thread.sleep(3000);
    ...
    private void deployAdaContract() throws Exception {
        log.info("[AdaCoinService] ADA ERC-20 토큰 컨트랙트 배포 중...");
        ...
        log.info("[AdaCoinService] 배포 트랜잭션 전송 완료. TXID: {}", txHash);
        ...
        log.info("[AdaCoinService] ADA ERC-20 토큰 계약 배포 성공! CA: {}", contractAddress);
    }

    @Override
    public String transfer(String toAddress, BigDecimal amount) throws Exception {
        if (!initialized || contractAddress == null) {
            throw new IllegalStateException("AdaCoinService is not initialized yet.");
        }

        log.info("[AdaCoinService] ADA 송금 요청: {} ADA -> {}", amount, toAddress);
        ...
        EthSendTransaction ethSendTransaction = web3j.ethSendRawTransaction(hexValue).send();
        if (ethSendTransaction.hasError()) {
            throw new RuntimeException("ADA transfer transaction failed: " + ethSendTransaction.getError().getMessage());
        }

        String txHash = ethSendTransaction.getTransactionHash();
        log.info("[AdaCoinService] ADA 송금 트랜잭션 브로드캐스트 성공. TXID: {}", txHash);
        return txHash;
    }
    ...
    public BigDecimal getBalance(String address) {
        ...
        try {
            ...
            log.error("[AdaCoinService] 잔고 조회 실패 ({}): {}", address, e.getMessage());
            return BigDecimal.ZERO;
        }
    }
    ...
    public boolean supports(String currency) {
        return "ADA".equalsIgnoreCase(currency);
    }
